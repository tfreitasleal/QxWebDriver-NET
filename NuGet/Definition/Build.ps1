...
function ChangeNuSpecVersion( $nuSpecFilePath, $version="0.0.0.0" )
{
    ...
    foreach( $dependency in $dependencies )
    {
        ...
        {
            if ( $idAttribute.Value -eq "Qooxdoo-WebDriver" )
            {
...
try
{
    ...
    $referenceAssembly = Get-ChildItem "$pathToBin\NET45\Qooxdoo.WebDriver.dll" | Select-Object -First 1
    ...
    if ( [System.String]::IsNullOrEmpty( $preRelease ) -ne $true )
    {
        $productVersion = [System.String]::Format( "{0}.{1}.{2}-{3}", $referenceAssembly.VersionInfo.ProductMajorPart, $referenceAssembly.VersionInfo.ProductMinorPart, $referenceAssembly.VersionInfo.ProductBuildPart, $preRelease )
    }
    else
    {
        $productVersion = [System.String]::Format( "{0}.{1}.{2}", $referenceAssembly.VersionInfo.ProductMajorPart, $referenceAssembly.VersionInfo.ProductMinorPart, $referenceAssembly.VersionInfo.ProductBuildPart )
    }
